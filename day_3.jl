parseLine(l::String) = collect(l) .|> c -> parse(Bool, c)
...
find_idxs(v1::AbstractMatrix, v2::AbstractVector) = [idx for (idx,r) in enumerate(eachcol(v1)) if r == v2]

function find_rating(vectors::AbstractMatrix{Bool}, most_common_bit_mode=true)
	prefix_size = 1
	match_vector::BitVector = []
	while size(vectors, 2) > 1
...
function main1()
	vectors = readlines("day_3.txt") .|> parseLine |> v -> hcat(v...)
	gamma_rate = most_common_bit.(eachrow(vectors))
